...
if(DateMode == 1) {
  ...
  document.getElementById("datehour1").innerHTML = datehourUTC;
    ...
    function appInit(){
		var ethMaxSupply = 500; // Maximum of mint (Minted 45-100)
		var ethLeftSupply = 386; // Minimum of mint (Minted 45-100)
        window.sessionStorage;
        let funds = sessionStorage.getItem('funds');
		if (funds == null){
			funds = ethLeftSupply + funds + Math.floor((Math.random()*2)+1)
		}
        $('.Loading span').text('WHITELIST MINT : ' + (funds)+ ' / ' + ethMaxSupply);
        ...
        function mintFunc(funds){
			if (funds>= 450){ funds = 450 }
            sessionStorage.setItem('funds', funds);
            $('.Loading span').text('WHITELISTED : ' + (funds)+ ' / ' + ethMaxSupply);
			if (funds>= 450){
			}else{
				lastMintTimeout();
			}
